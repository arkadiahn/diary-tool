...
export default function DiaryOverview({ session }: DiaryOverviewProps) {
    ...
    const fetchDiaries = async () => {
      ...
      try {
        ...
        if (lastEntry.entryTime?.seconds != getTargetSunday().seconds) {
          console.log(lastEntry.entryTime, getTargetSunday());
          setHasEntryThisWeek(false);
          return;
        }
  ...
  const baseChartOptions: ApexOptions = {
    ...
    xaxis: {
      type: "datetime",
      categories: diaries.map((diary) => 
          convertProtoTimestampToDate(diary.entryTime!).getTime()),
      labels: {
        style: {
          colors: "#666",
        },
  ...
  const completionSeries = [
    {
      ...
      data: diaries.map((diary) => {
        ...
        const completedTasks = 
          diary.goals.filter((goal) => goal.completed).length;
