...
export default function MissionLayout({ missions, accounts, milestones }: MissionLayoutProps) {
	...
	// const upcomingMissions = missions.filter((mission) => mission.state === Mission_State.ACTIVE && timestampToDate(mission.kickoffTime)! > new Date());
	const featuredMissions = missions.filter((mission) => mission.name.includes("m007") || mission.name.includes("m008"));
